...
class SettingsScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      // comparable screen (e.g. RegionDetailScreen, OnboardingScreen), this
      // Scaffold had no SafeArea at all - `/settings` and `/settings/about`
      // are top-level routes outside AppShell (see router.dart), so they
      // never inherit AppShell's own `SafeArea(top: false)` either. On a
      // platform/window where system chrome overlaps the bottom of the
      // viewport (e.g. a short desktop browser window with the OS taskbar
      // cutting into it, or a phone's gesture-nav bar), the last card - "Über
      // die App", with the hidden 7-tap dev-code unlock - could sit behind
      // that chrome with no way to scroll it fully into view. SafeArea plus
      // extra bottom padding on the ListView itself (so the last item can
      // scroll past any inset, not just up to it) fixes both platforms at
      // once.
      body: SafeArea(
        child: ListView(
